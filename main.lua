...
function fillPage(page)
	...
	gbmisc:setSize(138+58,32)

		local bgetcode = gbmisc:addItem(Button:new("B_GetCode"))
		bgetcode:setPosition(4,4)
		bgetcode.caption = "Save"
		bgetcode:setSize(48,24)
		function bgetcode:click(b)
			if b == "l" then
				local em = page:getItem("ParticleEmitter").ps
				local code = ""
				code = code.."shared_objects[2] = love.graphics.newParticleSystem(shared_objects[1],"..em:getBufferSize()..")\n"
				code = code.."shared_objects[2]:setDirection("..em:getDirection()..")\n"
				local as,asx,asy = em:getAreaSpread()
				code = code.."shared_objects[2]:setAreaSpread(\""..as.."\","..asx..","..asy..")\n"
				code = code.."shared_objects[2]:setEmissionRate("..em:getEmissionRate()..")\n"
				code = code.."shared_objects[2]:setEmitterLifetime("..em:getEmitterLifetime()..")\n"
				local elaxmin,elaymin,elaxmax,elaymax = em:getLinearAcceleration()
				code = code.."shared_objects[2]:setLinearAcceleration("..elaxmin..","..elaymin..","..elaxmax..","..elaymax..")\n"
				local pttlmin,pttlmax = em:getParticleLifetime()
				code = code.."shared_objects[2]:setParticleLifetime("..pttlmin..","..pttlmax..")\n"
				local praccmin,praccmax = em:getRadialAcceleration()
				code = code.."shared_objects[2]:setRadialAcceleration("..praccmin..","..praccmax..")\n"
				local protmin,protmax = em:getRotation()
				code = code.."shared_objects[2]:setRotation("..protmin..","..protmax..")\n"
				local ptgamin,ptgamax = em:getTangentialAcceleration()
				code = code.."shared_objects[2]:setTangentialAcceleration("..ptgamin..","..ptgamax..")\n"
				local pspdmin,pspdmax = em:getSpeed()
				code = code.."shared_objects[2]:setSpeed("..pspdmin..","..pspdmax..")\n"
				local pspinmin,pspinmax = em:getSpin()
				code = code.."shared_objects[2]:setSpin("..pspinmin..","..pspinmax..")\n"
				code = code.."shared_objects[2]:setSpinVariation("..em:getSpinVariation()..")\n"
				local pldmin,pldmax = em:getLinearDamping()
				code = code.."shared_objects[2]:setLinearDamping("..pldmin..","..pldmax..")\n"
				code = code.."shared_objects[2]:setSpread("..em:getSpread()..")\n"
				code = code.."shared_objects[2]:setRelativeRotation("..tostring(em:hasRelativeRotation())..")\n"
				local ox,oy = em:getOffset()
				code = code.."shared_objects[2]:setOffset("..ox..","..oy..")\n"
				local sizes = ""
				for i=1,spsrange.value do
					sizes = sizes..rbszgr[i].caption
					if i~=spsrange.value then sizes = sizes.."," end
				end
				code = code.."shared_objects[2]:setSizes("..sizes..")\n"
				code = code.."shared_objects[2]:setSizeVariation("..em:getSizeVariation()..")\n"
				local colors = ""
				...
				code = code.."shared_objects[2]:setColors("..colors..")\n"
				if cbuseq.checked == true and #lbqlist.items>0 then
					...
					code = code..qlist
					code = code.."shared_objects[2]:setQuads("..qvarlist..")".."\n"
				end
				print(em)
				love.system.setClipboardText(code)
				success = love.filesystem.write( "particule.lua", code)
			end
		end

		local bgetcode = gbmisc:addItem(Button:new("B_Load"))
		bgetcode:setPosition(54,4)
		bgetcode.caption = "Load"
		bgetcode:setSize(54,24)
		function bgetcode:click(b)
			if b == "l" then
				contents, size = love.filesystem.read("particule.lua")
				print(contents)
				local em = page:getItem("ParticleEmitter").ps
				shared_objects  = {em:getTexture()}
				print(shared_objects[1],"qsdqsd")
				test = loadstring(contents)
				test()
				print(shared_objects[2],"AAAA")
				local t = page:getItem("ParticleEmitter")
				t:aaaaaaaa(shared_objects[2])
			end
		end
